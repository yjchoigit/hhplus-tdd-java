...
public class PointValidationService {
    ...
    public void checkId(Long id) {
        if (id == null || id <= 0) {
            throw new IllegalArgumentException(PointEnums.Error.ID.getMsg());
        }
    }

    // 포인트는 null이나 0일 수 없음
    public void checkAmount(TransactionType transactionType, Long amount) {
        // TODO transactionType에 따른 amount valid 추가 가능성...
        // 현재는 기본만 null or 0
        if (amount == null || amount == 0) {
            throw new IllegalArgumentException(PointEnums.Error.AMOUNT.getMsg());
        }
    }
